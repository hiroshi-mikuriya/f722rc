...
private:
    ...
    const string paramName[20] = { "LEVEL", "MIX", "F.BACK", "RATE", "DEPTH",
        "TONE"
        "",
        "", "", "", "", "", "", "", "", "", "", "", "", "" };
...
public:
    ...
    virtual void init() {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (fxAllData[fxNum][i] < paramMin[i] || fxAllData[fxNum][i] > paramMax[i])
                fxParam[i] = paramMin[i];
            else
                fxParam[i] = fxAllData[fxNum][i];
        }
    ...
    virtual void deinit() { del1.erase(); }
