...
uint8_t ssd1306_Init(I2C_HandleTypeDef* hi2c) {
    ...
    status +=
        ssd1306_WriteCommand(hi2c, 0x10); // 00,Horizontal Addressing Mode;01,Vertical Addressing
    ...
    status += ssd1306_WriteCommand(
        hi2c, 0xA4); // 0xa4,Output follows RAM content;0xa5,Output ignores RAM content
...
void ssd1306_UpdateScreen(I2C_HandleTypeDef* hi2c) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        HAL_I2C_Mem_Write(hi2c, SSD1306_I2C_ADDR, 0x40, 1, &SSD1306_Buffer[SSD1306_WIDTH * i],
            SSD1306_WIDTH, 100);
    }
}
    ...
    */
    ...
    for (i = 0; i < Font.FontHeight; i++) {
        ...
        for (j = 0; j < Font.FontWidth; j++) {
            if ((b << j) & 0x8000) {
                ssd1306_DrawPixel(
                    SSD1306.CurrentX + j, (SSD1306.CurrentY + i), (SSD1306_COLOR)color);
            }
            else {
                ssd1306_DrawPixel(
                    SSD1306.CurrentX + j, (SSD1306.CurrentY + i), (SSD1306_COLOR)!color);
...
void ssd1306_InvertColors(void) { SSD1306.Inverted = !SSD1306.Inverted; }
