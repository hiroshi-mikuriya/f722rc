...
const uint8_t cursorPositionXY[6][2] = { { 0, 11 }, { 0, 29 }, { 0, 47 }, { 65, 11 }, { 65, 29 },
    { 65, 47 } }; // カーソル位置
const uint8_t fxParamNameXY[6][2] = { { 0, 17 }, { 0, 35 }, { 0, 53 }, { 65, 17 }, { 65, 35 },
    { 65, 53 } }; // エフェクトパラメータ名表示位置
const uint8_t fxParamStrXY[6][2] = { { 52, 11 }, { 52, 29 }, { 52, 47 }, { 117, 11 }, { 117, 29 },
    { 117, 47 } }; // エフェクトパラメータ数値表示 右端の文字位置
    ...
    case 0: // 左上スイッチ --------------------------------------------------------
        if (!HAL_GPIO_ReadPin(SW0_UPPER_L_GPIO_Port, SW0_UPPER_L_Pin)) {
                ...
                swCount[num] < longPushCount) // 短押し 離した時の処理
            {
                ...
                else { // エフェクトパラメータ選択位置変更 0→最大値で循環
    ...
    case 1: // 左下スイッチ --------------------------------------------------------
        if (!HAL_GPIO_ReadPin(SW1_LOWER_L_GPIO_Port, SW1_LOWER_L_Pin)) {
                ...
                swCount[num] < longPushCount) // 短押し 離した時の処理
            {
                ...
                else { // エフェクトパラメータ選択位置変更 最大値→0で循環
...
int32_t swap16(int32_t x) { return (0x0000FFFF & x >> 16) | x << 16; }
